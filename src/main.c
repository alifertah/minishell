...
/*   Updated: 2022/11/27 20:00:28 by olabrahm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_next_arg_file(t_ptree_nodes *nodes, t_ptree_iters *iters, char **cmd)
{
	...
	if ((nodes->current_node)->token == HEREDOC)
	{
		...
		(nodes->current_node)->fd = ft_heredoc(&g_state,
				(nodes->current_node)->eof);
...
void	ft_set_home(t_env_var *new_value)
{
	g_state.home = new_value;
}
